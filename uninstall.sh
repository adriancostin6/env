...
log "starting uninstaller." | tee -a "$ENV_LOGFILE"

ENV_CACHE_DIR="$HOME/.local/share/env"
if [ ! -f "$ENV_CACHE_DIR/install.lock" ]; then
  die "already clean, exiting." | tee -a "$ENV_LOGFILE"
fi
log "removing $ENV_CACHE_DIR" | tee -a "$ENV_LOGFILE"
rm -rf "$ENV_CACHE_DIR"

env_uninstall() {
  dbg "running $1/uninstall.sh" | tee -a "$ENV_LOGFILE"
  pushd $1
  bash ./uninstall.sh
  popd
}

#log "uninstalling tools" | tee -a "$ENV_LOGFILE"
...
rm_symlink() {
  dbg "removing symlink $1" | tee -a "$ENV_LOGFILE"
  rm -f $1
}
CONFIG="$HOME/.config"
